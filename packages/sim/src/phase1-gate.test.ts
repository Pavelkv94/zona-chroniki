 ...
 * ТОЧНО тот же, что в headless CLI (createSimWorld → worldgen → регистрация систем
 * → scheduler.run), поэтому гейт стоит на настоящем конвейере, а не на фикстуре.
 *
 * ── ВЕСЬ ГЕЙТ на РЕАЛЬНОМ конвейере Фазы 3 (D-075: перф-квадрат снят) ──────────
 * Задача 3.7 (D-074) подключила НАРРАТИВНЫЙ БЛОК (Radio→Rumors→Chronicle) в конвейер (Фаза
 * 3, 20 систем, registerPhase3Systems), но на плотном старте rumor-память раздувалась до
 * квадрата в addMemory (memMax ~38k записей/NPC), делая 10-дневный прогон Фазы 3 непрактичным
 * ⇒ D-074 ВРЕМЕННО гонял дорогие 10-дневные профили на поведенчески-эквивалентном прокси
 * Фазы 2, а Фаза-3-факты анкерил на день-1. Задача 3.8 (D-075) сняла квадрат СТРУКТУРНО:
 * addMemory КОНСОЛИДИРУЕТ память по факту (kind, subject, isFirsthand) — memMax ~60, 10-
 * дневный прогон Фазы 3 снова дёшев. Поэтому ВЕСЬ гейт (профили, детерминизм, resume, анкеры)
 * стоит на НАСТОЯЩЕМ конвейере Фазы 3 (`buildLive` = registerPhase3Systems), а не на прокси.
 * Нарратив ПОВЕДЕНЧЕСКИ ИНЕРТЕН для физики (читает ЗАКОММИЧЕННОЕ прошлое `bus.at(...)`, пишет
 * ТОЛЬКО fame/memory — ключи ResourceStore, ДИЗЪЮНКТНЫЕ positions/inventory/Alive) ⇒ физика
 * (позиции/смерти/восприятие/масса) тождественна Фазе 2 (perception/spotted = 7224 после 5.2, MUST-9).
 */
...
import { registerPhase3Systems, PHASE3_SYSTEMS } from './pipeline';
 ...
 */
const GOLDEN_DAY1_SEED42 = '6d4317ab'; // 5.2/D-085 (FORAGE→forage_food): ПОВЕДЕНЧЕСКИЙ сдвиг — фуражировка снимает голодные смерти дня-1 (день 1 стал мирным, охота на стада −77%) ⇒ 1b5beda6 → 6d4317ab (лог день-1 14794→18814; детерминизм 2×). // 5.0/D-083 (схемы Фазы 5): рост singleton WorldClock 2→5 полей сдвинул канон ЧИСТО СХЕМНО 429867e2 → 1b5beda6. // 3.8/D-075 (фикс здоровья нарратива): addMemory КОНСОЛИДИРУЕТ память по (kind, subject, isFirsthand) — один слух/факт = одна ОСВЕЖАЕМАЯ запись, а не тысячи копий (снят квадрат rumor-памяти 3.7). Event-ЛОГ БАЙТ-В-БАЙТ тот же (events 14794 не изменились — дедуп трогает ТОЛЬКО ключ 'memory', поведение мира тождественно, perception/spotted по-прежнему 9217), но сериализованная 'memory' иная ⇒ хэш сдвинут f554331d → 429867e2. (ранее 3.7/D-074: 3c54d141 → f554331d; 3.3/D-071: 9bc823a7 → 3c54d141)
...
// ─────────────────────────────────────────────────────────────────────────────
// СБОРКА ЖИВОГО МИРА — РЕАЛЬНЫЙ конвейер Фазы 3 (20 систем, registerPhase3Systems —
// тот же путь, что headless-CLI). Задача 3.8 (D-075) ВЕРНУЛА гейт на Фазу 3: квадрат
// rumor-памяти 3.7 снят структурно (addMemory консолидирует память по факту), поэтому
// 10-дневные физические профили снова гоняются на НАСТОЯЩЕМ шипящемся конвейере, а не на
// поведенчески-эквивалентном прокси Фазы 2 (как было при ПЕРФ-ФЛАГЕ D-074). Нарратив
// поведенчески инертен для физики (читает закоммиченное прошлое, пишет fame/memory —
// дизъюнктные positions/inventory/Alive) ⇒ физические профили те же, что и на Фазе 2.
// ─────────────────────────────────────────────────────────────────────────────
...
describe('MUST · Resume: split save/load ≡ непрерывный прогон (несколько горизонтов)', () => {
  ...
  for (const [seed, totalDays, splitDays] of CASES) {
    describe(`seed ${seed}: ${totalDays} дней, сплит на дне ${splitDays}`, () => {
      ...
      registerPhase3Systems(resumeSched); // резюм-конвейер = тот же Фаза 3, что cont/half (D-075)
...
// ═════════════════════════════════════════════════════════════════════════════
describe('MUST · Порядок систем: живой конвейер Фазы 3 удовлетворяет стыки причинности (D-032/D-074)', () => {
  ...
  it('зарегистрированы ровно 20 систем Фазы 3 в каноническом порядке', () => {
    expect(order).toEqual(PHASE3_SYSTEMS.map((s) => s.name));
    expect(order.length).toBe(20);
  });

  it('производитель < потребитель на всех стыках Фазы 1+2+нарратив (D-032/D-064/D-074)', () => {
    ...
    // Нарративный блок Фазы 3 (D-074): MemoryDecay < Radio < Rumors < Chronicle < Death.
...
describe('MUST · Целостность: конвейер не падает и держит живой голден', () => {
  it('день-1/seed-42 через РЕАЛЬНЫЙ конвейер Фазы 3 даёт голден 429867e2 (D-075)', () => {
    // Анкер на РЕАЛЬНОМ конвейере Фазы 3 (buildLive = registerPhase3Systems, тот же путь,
    // что headless-CLI): с нарративным блоком + консолидацией rumor-памяти (D-075) хэш день-1
    // = 429867e2 (см. GOLDEN_DAY1_SEED42 выше — тот же голден, что cli.test day1 seed42).
...
// Ф3-конвейере, тем же путём, что headless-CLI.
// ═════════════════════════════════════════════════════════════════════════════
describe('MUST · Гейт Perception по актёрам: не-акторы (поселения+поля) вне spotted (P-2, D-072)', () => {
  ...
  it('счётчик perception/spotted за день-1 seed42 ЗАФИКСИРОВАН числом: 7224 (шум не-акторов убран, D-072)', () => {
    ...
    // Задача 3.7 (D-074) / 3.8 (D-075): анкер на РЕАЛЬНОМ конвейере Фазы 3 (buildLive). Нарратив
    // (Radio/Rumors/Chronicle) читает ЗАКОММИЧЕННОЕ прошлое и пишет ТОЛЬКО fame/memory (ключи,
    // дизъюнктные positions/perception) ⇒ поведение мира и число perception/spotted НЕ меняются:
    // по-прежнему совпадает с физическим конвейером. 5.2/D-085 (FORAGE→forage_food): фуражировка
    // меняет ранние траектории (люди собирают, а не только охотятся; меньше ранних смертей),
    // ⇒ иной детерминированный набор встреч дня-1: perception/spotted 9217 → 7224. НЕ регресс:
    // восприятие акторов работает, ни один spotted не про не-актора (проверено ниже).
    const { world, scheduler } = buildLive(42);
    scheduler.run(world, TICKS_PER_DAY);
    const spotted = world.bus.log.filter((e) => e.type === 'perception/spotted');
    expect(spotted).toHaveLength(7224);
