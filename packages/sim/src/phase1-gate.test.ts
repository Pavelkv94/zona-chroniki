 ...
 * Пере-закреплён задачей 2.6: TaskSelection выбирает TRADE (NPC со спросом/избытком
 * идут торговать к поселениям) — новые задачи/переходы двигают историю → 70e9e546
 * → 165688eb. Trade-исполнение не в конвейере (D-047), сделок нет — сдвиг только от
 * распорядка задач.
 */
const GOLDEN_DAY1_SEED42 = '165688eb';
...
describe('MUST · Целостность: конвейер не падает и держит живой голден', () => {
  it('день-1/seed-42 через тот же путь сборки даёт голден 165688eb (D-056)', () => {
