 ...
 */
const GOLDEN_DAY1_SEED42 = '429867e2'; // 3.8/D-075 (фикс здоровья нарратива): addMemory КОНСОЛИДИРУЕТ память по (kind, subject, isFirsthand) — один слух/факт = одна ОСВЕЖАЕМАЯ запись, а не тысячи копий (снят квадрат rumor-памяти 3.7). Event-ЛОГ БАЙТ-В-БАЙТ тот же (events 14794 не изменились — дедуп трогает ТОЛЬКО ключ 'memory', поведение мира тождественно, perception/spotted по-прежнему 9217), но сериализованная 'memory' иная ⇒ хэш сдвинут f554331d → 429867e2. (ранее 3.7/D-074: 3c54d141 → f554331d; 3.3/D-071: 9bc823a7 → 3c54d141)
