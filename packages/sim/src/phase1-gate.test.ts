 ...
 * (позиции/смерти/восприятие/масса) тождественна Фазе 2 (perception/spotted = 9217, MUST-9).
 */
 ...
 */
const GOLDEN_DAY1_SEED42 = '1b5beda6'; // 5.0/D-083 (схемы Фазы 5): singleton WorldClock вырос с 2→5 полей (эмиссия, все=0) ⇒ канон сдвинут ЧИСТО СХЕМНО 429867e2 → 1b5beda6 (events 14794 те же, лог байт-в-байт; пере-сериализация со старым набором полей воспроизводит 429867e2 — поведение не тронуто). // 3.8/D-075 (фикс здоровья нарратива): addMemory КОНСОЛИДИРУЕТ память по (kind, subject, isFirsthand) — один слух/факт = одна ОСВЕЖАЕМАЯ запись, а не тысячи копий (снят квадрат rumor-памяти 3.7). Event-ЛОГ БАЙТ-В-БАЙТ тот же (events 14794 не изменились — дедуп трогает ТОЛЬКО ключ 'memory', поведение мира тождественно, perception/spotted по-прежнему 9217), но сериализованная 'memory' иная ⇒ хэш сдвинут f554331d → 429867e2. (ранее 3.7/D-074: 3c54d141 → f554331d; 3.3/D-071: 9bc823a7 → 3c54d141)
...
describe('MUST · Гейт Perception по актёрам: не-акторы (поселения+поля) вне spotted (P-2, D-072)', () => {
  ...
  it('счётчик perception/spotted за день-1 seed42 ЗАФИКСИРОВАН числом: 9217 (шум не-акторов убран, D-072)', () => {
    ...
    // по-прежнему 9217 на конвейере с нарративом. Это ПРЯМОЕ доказательство поведенческой
    // инертности нарратива для физики (и потому весь гейт корректно стоит на Фазе 3, D-075).
    const { world, scheduler } = buildLive(42);
    scheduler.run(world, TICKS_PER_DAY);
