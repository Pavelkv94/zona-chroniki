 ...
 *
 * СЦЕНАРИЙ МИРА (QA 2.2; обновлён 3.0/D-072, P-2): «Поселение стоит посреди живой
 * Зоны, но остаётся ИНЕРТНЫМ». Поселение — сущность с `Position` (D-046), но НЕ актёр
 * (нет Human/Animal). После гейта Perception по актёрам (P-2, D-072) поселение вообще
 * НЕ попадает в бакеты восприятия — соседи его не видят, само оно никого не
 * воспринимает. Задача этого гейта — доказать инертность: за 30 дней полного
 * конвейера Фазы 1 поселение
 ...
 *
 * И ПОДТВЕРЖДАЕМ гейт P-2 (D-072): поселение НЕ появляется в `contacts` соседей и
 * само не наблюдает — никакого шумового `perception/spotted` про базу (ранее этот
 * шум ПИНОВАЛСЯ как безвредный; P-2 его законно устранил до Фазы 3).
 *
...
describe('поселение инертно: 30 дней живой Зоны не превращают базу в актора (2.2, D-046)', () => {
  ...
  it('ГЕЙТ P-2 (D-072): поселение НЕ видно соседям и само никого не воспринимает', () => {
    // Поселение — носитель Position, но НЕ актёр (нет Human/Animal). Гейт Perception
    // по актёрам (P-2, D-072) исключает его из бакетов: ни один co-located актёр не
    // держит поселение в contacts, и само поселение contacts не получает (в бакет не
    // попало) ⇒ никаких шумовых perception/spotted про базу. Ранее этот тест ПИНОВАЛ
    // обратное (поселение видно соседям) как известный шум — P-2 его законно устранил.
    const { world, scheduler } = buildLive(42);
    const bases = new Set(settlementEids(world));
    scheduler.run(world, 3); // достаточно, чтобы Perception построил contacts

    // Ни один актёр не «замечает» поселение.
    ...
    expect(seenBySomeNeighbour).toBe(false);

    // И само поселение в бакет не попало ⇒ contacts ему не пишутся (не наблюдатель).
    for (const s of bases) {
      expect(world.resources.has('contacts', s)).toBe(false);
    }

    // Ни одного perception/spotted, где поселение — наблюдатель ИЛИ цель.
    const noiseSpotted = world.bus.log.filter(
      (e) =>
        e.type === 'perception/spotted' &&
        (bases.has((e.payload as { observer: EntityId }).observer) ||
          bases.has((e.payload as { target: EntityId }).target)),
    );
    expect(noiseSpotted).toHaveLength(0);
  }, 15000);
});
