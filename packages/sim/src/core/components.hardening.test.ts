...
describe('таксономия TaskKind: append-only, плотная, ui8 (структура, не контент)', () => {
  it('текущий набор кодов зафиксирован и стабилен (append-only — новые в КОНЕЦ)', () => {
    ...
    expect(Object.entries(TaskKind).sort((a, b) => a[1] - b[1])).toEqual([
      ...
      ['REST', 5],
      ['FLEE', 6],
      // Фаза 2 (задача 2.1/2.8): append-only коды 7–10 (работа/торговля/грабёж/поход).
      ['WORK', 7],
      ['TRADE', 8],
      ['ROB', 9],
      ['SEARCH', 10],
      // Фаза 5 (задача 5.0): append-only коды 11–12 (укрыться от выброса / лечить).
      // В 5.0 ни один utility их не выбирает — чисто расширение кодового пространства.
      ['TAKE_SHELTER', 11],
      ['TREAT', 12],
    ]);
  });
