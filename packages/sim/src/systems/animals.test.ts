...
/**
 * Пишет contacts в форме 1.10a (`Contact[]`, сорт. по target): каждая запись —
 * `[target, spottedEvent]`. `spottedEvent` (числовой EventId, 0 = нет id) даёт
 * бегству причину для `move/departed.causedBy` (D-030).
 */
function setContacts(
  world: SimWorld,
  eid: EntityId,
  entries: ReadonlyArray<readonly [EntityId, number]>,
): void {
  world.resources.set(
    'contacts',
    eid,
    entries.map(([target, spottedEvent]) => ({ target, spottedEvent })),
  );
}
...
describe('бегство пугливых от людей (закон №2, D-029)', () => {
  ...
  it('flee-причинность ЗАМКНУТА (1.10a/D-037): move/departed.causedBy = spottedEvent человека', () => {
    const w = createSimWorld(50 as Seed);
    const loc = 1;
    const deer = placeAnimal(w, DEER, 1, loc);
    const human = placeHuman(w, loc);
    // Контакт несёт id perception/spotted (например 777) — бегство обязано на него сослаться.
    const SPOTTED = 777;
    setContacts(w, deer, [[human, SPOTTED]]);

    scheduler(Animals).run(w, 1);

    const departed = w.bus.log.filter((e) => e.type === 'move/departed');
    expect(departed).toHaveLength(1);
    // Причина departure бегства — РОВНО spottedEvent человека-угрозы (НЕ null).
    expect(departed[0]!.causedBy).toBe(SPOTTED);
    expect(departed[0]!.causedBy).not.toBeNull();
  });

  it('flee при spottedEvent=0 (нет id) → causedBy = null (корень), но бежит', () => {
    const w = createSimWorld(51 as Seed);
    const loc = 1;
    const deer = placeAnimal(w, DEER, 1, loc);
    const human = placeHuman(w, loc);
    setContacts(w, deer, [[human, 0]]); // человек есть, но id причины нет

    scheduler(Animals).run(w, 1);

    const departed = w.bus.log.filter((e) => e.type === 'move/departed');
    expect(departed).toHaveLength(1); // всё равно бежит
    expect(departed[0]!.causedBy).toBeNull(); // но причина не проставлена
  });

  it('ЦЕПОЧКА Perception→flee резолвится: departed.causedBy → реальный perception/spotted', () => {
    // Полный конвейер: Perception пишет contacts со spottedEvent, Animals читает его и
    // бежит, ссылаясь на то же perception/spotted (человек в поле зрения оленя).
    const w = createSimWorld(52 as Seed);
    const loc = 1;
    const deer = placeAnimal(w, DEER, 1, loc);
    const human = placeHuman(w, loc);

    // Порядок: Perception (producer) < Animals (consumer), D-032.
    scheduler(Perception, Animals).run(w, 1);

    // Perception опубликовал spotted оленя про человека.
    const spotted = w.bus.log.filter(
      (e) => e.type === 'perception/spotted' && (e.payload as { observer: number }).observer === deer,
    );
    expect(spotted).toHaveLength(1);
    expect((spotted[0]!.payload as { target: number }).target).toBe(human);

    // Animals убежал, и причина departure РЕЗОЛВИТСЯ к этому perception/spotted.
    const departed = w.bus.log.filter(
      (e) => e.type === 'move/departed' && (e.payload as { eid: number }).eid === deer,
    );
    expect(departed).toHaveLength(1);
    expect(departed[0]!.causedBy).toBe(spotted[0]!.id);
    const cause = w.bus.log.find((e) => e.id === departed[0]!.causedBy);
    expect(cause!.type).toBe('perception/spotted');
  });
...
describe('бегство — детерминизм направления и краевые случаи', () => {
  ...
  it('бегство ПРИОРИТЕТНЕЕ стадности: отставший олень с угрозой бежит от людей, не к стаду', () => {
    ...
    setContacts(w, straggler, [[human, 0]]);
    scheduler(Animals).run(w, 1);
