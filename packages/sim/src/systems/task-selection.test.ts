...
import { TaskSelection, STRICT_HUNT_PERCEPTION } from './task-selection';
import { setRelation, entitySubject, addAvoid, getAvoids, isAvoided } from './memory';
import { updateHuntMemory } from './hunt-memory';
...
/**
 * Записывает в ЛИЧНУЮ охотничью память `hunter` угодья `locs` (как будто он ЛИЧНО там
 * видел дичь, D-087, анти-чит). Нужно там, где тест проверяет HUNT-таргетинг/обход/погоню
 * БЕЗ прогона Perception (evalAt): после P-5/Б кандидаты-угодья берутся из восприятия+
 * памяти, а НЕ из глобального скана животных, поэтому охотник должен «знать» угодье. Seed
 * идёт на текущем `world.tick` (0 на свежем мире) — угодье свежее, expectation=1.
 */
function rememberGrounds(world: SimWorld, hunter: EntityId, ...locs: number[]): void {
  updateHuntMemory(world.resources, hunter, world.tick, new Set(locs), undefined);
}
...
/**
 * Планировщик Perception→TaskSelection (реальный порядок пайплайна): Perception
 * наполняет `contacts` ДО выбора задачи, поэтому охотник ВИДИТ co-located дичь и может
 * выбрать её ЛИЧНО ВОСПРИНЯТОЙ жертвой (задача A P-5, анти-чит). Нужен там, где важен
 * targetEid охоты (без Perception contacts пусты ⇒ targetEid=0, охотник «в пути»).
 */
function perceiveScheduler(): Scheduler {
  const s = createScheduler();
...
/** Perception+TaskSelection РОВНО один раз на тике `tick`. */
function perceiveEvalAt(world: SimWorld, tick: Tick): void {
  world.tick = tick;
  perceiveScheduler().tickOnce(world);
}
...
describe('объяснимость: выбор выводится из состояния (закон №2)', () => {
  ...
  it('голодный БЕЗ еды, дичь в соседней локации → HUNT (target=угодье; жертва — в пути 0)', () => {
    const w = createSimWorld(2 as Seed);
    const eid = placeStalker(w, { loc: 3, hunger: 85, thirst: 10, fatigue: 10, survival: 0.6, food: false });
    placeAnimal(w, 4); // сосед loc3 (ребро 3—4) — дичь НЕ co-located, охотник её ещё не видит
    rememberGrounds(w, eid, 4); // но он ПОМНИТ угодье loc4 (личное прошлое наблюдение, D-087)
    evalAt(w, DAY_TICK);
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT);
    expect(TSK.targetLoc[eid]).toBe(4); // локация-угодье (куда идти)
    // Задача A P-5 (анти-чит): пока охотник В ПУТИ к угодью, он дичь ЛИЧНО не воспринимает
    // ⇒ targetEid=0 (не «застолблена» глобально); жертву свяжет Encounters по прибытии.
    expect(TSK.targetEid[eid]).toBe(0);
  });
...
describe('обход маршрута: NPC избегает помеченную локацию при выборе цели', () => {
  ...
  it('HUNT: избегаемую локацию дичи НЕ выбирает — уходит на дальнюю неизбегаемую', () => {
    ...
    placeAnimal(w, 2);
    placeAnimal(w, 3);
    rememberGrounds(w, eid, 2, 3); // помнит оба угодья (личный опыт, D-087)
  ...
  it('HUNT: единственная дичь в избегаемой локации ⇒ HUNT исключён (−∞), выбор иной задачи', () => {
    ...
    rememberGrounds(w, eid, 2); // помнит это угодье (личный опыт, D-087)
  ...
  it('обход НЕ влияет на NPC без avoid-записей (изоляция)', () => {
    ...
    placeAnimal(w, 2);
    placeAnimal(w, 3);
    rememberGrounds(w, avoider, 2, 3); // оба помнят оба угодья (личный опыт, D-087)
    rememberGrounds(w, clean, 2, 3);
...
describe('детерминизм: один seed → идентичные Task и лог task/selected', () => {
  ...
  function runMixed(seed: number): { events: unknown[]; kinds: number[] } {
    ...
    const hunter = placeStalker(w, { loc: 3, hunger: 90, survival: 0.6, food: false }); // HUNT (дичь ниже)
    ...
    rememberGrounds(w, hunter, 4); // охотник помнит угодье loc4 (личный опыт, D-087)
    evalAt(w, DAY_TICK);
    const events = w.bus.log
      .filter((e) => e.type === 'task/selected')
...
describe('интеграция: task/selected → move/departed → move/arrived (штампы, без скана)', () => {
  it('HUNT в соседнюю локацию: departed.causedBy=task/selected, arrived.causedBy=departed', () => {
    ...
    rememberGrounds(w, eid, 4); // помнит угодье loc4 (личный опыт, D-087)
...
// ───────────────────────────────────────────────────────────────────────────
describe('HUNT: цель из ЛИЧНО ВОСПРИНЯТОГО; смерть жертвы вызывает перевыбор (D-029, A P-5)', () => {
  it('дичь co-located → HUNT target=loc, targetEid — ВОСПРИНЯТАЯ особь (одна из стада), на месте', () => {
    ...
    const preyA = placeAnimal(w, 4);
    perceiveEvalAt(w, DAY_TICK); // Perception наполняет contacts → охотник ВИДИТ стадо
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT);
    expect(TSK.targetLoc[eid]).toBe(4);
    // Жертва — из ЛИЧНОГО восприятия (одна из co-located особей), НЕ глобальный min-eid.
    expect([preyA, preyB]).toContain(TSK.targetEid[eid] as EntityId);
  });

  it('жертва умерла → targetEid обнулён СРАЗУ; пустые выходы забывают угодье → HUNT сходит (D-087)', () => {
    const w = createSimWorld(221 as Seed);
    const eid = placeStalker(w, { loc: 4, hunger: 92, thirst: 5, fatigue: 5, survival: 0.7, food: false });
    const prey = placeAnimal(w, 4); // единственная дичь, co-located
    const sched = perceiveScheduler();
    w.tick = DAY_TICK;
    sched.tickOnce(w);
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT);
    expect(TSK.targetEid[eid]).toBe(prey); // одна воспринятая особь ⇒ она и есть цель

    // Зверь гибнет (Encounter 1.10 снял бы Alive). Живой дичи в мире не осталось.
    removeComponent(w.ecs, Alive, prey);
    w.tick = DAY_TICK;
    sched.tickOnce(w);
    // СРАЗУ: ЖИВОЙ воспринятой жертвы нет ⇒ targetEid обнулён — «не залипает на покойнике»
    // (D-029). НО угодье охотник ещё ПОМНИТ (вера убывает от ПУСТЫХ выходов, а не мгновенно,
    // задача Б: недоохота ЭКОНОМИЧНА из личного опыта, а не из мирового скана «дичи нет»).
    ...
    // Пустые выходы к угодью без дичи ЭРОДИРУЮТ веру; через N выходов угодье ЗАБЫТО ⇒
    // HUNT выпадает из argmax и охотник ПЕРЕКЛЮЧАЕТСЯ (экономическая недоохота, задача Б).
    let switched = false;
    for (let t = 0; t < 12 && !switched; t++) {
      w.tick = DAY_TICK;
      sched.tickOnce(w);
      if ((TSK.kind[eid] as number) !== TaskKind.HUNT) switched = true;
    }
    expect(switched).toBe(true); // не залипает ВЕЧНО на перевыбитом угодье (Б)
  });

  it('жертва умерла, но рядом ещё дичь → HUNT перецеливается на ЖИВУЮ воспринятую', () => {
    ...
    w.tick = DAY_TICK;
    sched.tickOnce(w);
    const firstTarget = TSK.targetEid[eid] as EntityId;
    expect([prey1, prey2]).toContain(firstTarget); // воспринятая особь

    removeComponent(w.ecs, Alive, firstTarget); // выбранная жертва гибнет
    w.tick = DAY_TICK;
    sched.tickOnce(w);
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT); // дичь ещё есть — HUNT сохраняется
    const survivor = firstTarget === prey1 ? prey2 : prey1;
    expect(TSK.targetEid[eid]).toBe(survivor); // перецелился на ЖИВУЮ воспринятую особь
  });
});

// ───────────────────────────────────────────────────────────────────────────
// ЗАДАЧА A ЗАКРЫТИЯ P-5: анти-чит таргетинг, размазывание по стаду, стоимость погони
// ───────────────────────────────────────────────────────────────────────────
describe('P-5/A: цель охоты из ЛИЧНО ВОСПРИНЯТОГО (анти-чит), НЕ глобальный min-eid', () => {
  it('таргетинг читает ТОЛЬКО contacts: жертва — воспринятая особь, а не min-eid дичь мира', () => {
    const w = createSimWorld(300 as Seed);
    // Порядок спавна ⇒ eid: hunter < preyLow < preyHigh. Обе co-located в loc4.
    const hunter = placeStalker(w, { loc: 4, hunger: 90, survival: 0.6, food: false });
    const preyLow = placeAnimal(w, 4); // ГЛОБАЛЬНЫЙ min-eid среди животных
    const preyHigh = placeAnimal(w, 4);
    // Впрыскиваем восприятие ВРУЧНУЮ: охотник видит ТОЛЬКО preyHigh (preyLow — вне
    // contacts, хотя физически рядом и с меньшим eid). Без Perception контакты не
    // перезаписываются ⇒ таргетинг обязан взять жертву ИЗ ЭТОГО списка.
    w.resources.set('contacts', hunter, [{ target: preyHigh, spottedEvent: 1 as Contact['spottedEvent'] }]);
    evalAt(w, DAY_TICK);
    expect(TSK.kind[hunter]).toBe(TaskKind.HUNT);
    // Цель — воспринятая preyHigh, НЕ preyLow (доказ.: функция таргетинга не сканирует
    // всех животных мира и не берёт глобальный min-eid, а читает только contacts).
    expect(TSK.targetEid[hunter]).toBe(preyHigh);
    expect(TSK.targetEid[hunter]).not.toBe(preyLow);
  });

  it('два co-located охотника НЕ бьют гарантированно одну особь (давление размазано)', () => {
    function run(): { t1: number; t2: number; k1: number; k2: number } {
      const w = createSimWorld(301 as Seed);
      const h1 = placeStalker(w, { loc: 4, hunger: 90, survival: 0.6, food: false });
      const h2 = placeStalker(w, { loc: 4, hunger: 90, survival: 0.6, food: false });
      placeAnimal(w, 4);
      placeAnimal(w, 4);
      placeAnimal(w, 4); // стадо из 3 воспринятых особей
      perceiveEvalAt(w, DAY_TICK); // Perception → оба охотника видят всё стадо
      return {
        t1: TSK.targetEid[h1] as number,
        t2: TSK.targetEid[h2] as number,
        k1: TSK.kind[h1] as number,
        k2: TSK.kind[h2] as number,
      };
    }
    const a = run();
    expect(a.k1).toBe(TaskKind.HUNT);
    expect(a.k2).toBe(TaskKind.HUNT);
    // Размазывание по стаду: разные охотники → РАЗНЫЕ особи (не конвейерный ганг).
    expect(a.t1).not.toBe(a.t2);
    // Детерминизм 2× (закон №8): повтор даёт те же цели.
    const b = run();
    expect(b.t1).toBe(a.t1);
    expect(b.t2).toBe(a.t2);
  });
});

describe('P-5/A: стоимость погони дисконтирует охоту за далёкой/сбежавшей дичью (закон №2)', () => {
  // Одинаковый охотник в Кордоне (loc0): дичь РЯДОМ (loc1, dist 40) → HUNT; та же дичь
  // ДАЛЕКО (loc9, dist 355) → штраф погони гасит sHunt ниже fallback/SLEEP ⇒ НЕ гонит
  // через карту, переключается (даже несмотря на БОЛЬШЕЕ обилие дичи в loc9). Разница —
  // ТОЛЬКО дистанция (стоимость погони из состояния: путь + усталость), не таймер/шанс.
  function huntChoiceWithGameAt(gameLoc: number): number {
    const w = createSimWorld(302 as Seed);
    const eid = placeStalker(w, { loc: 0, hunger: 100, thirst: 5, fatigue: 0, survival: 0.5, food: false });
    placeAnimal(w, gameLoc);
    rememberGrounds(w, eid, gameLoc); // помнит угодье (личный опыт, D-087)
    evalAt(w, DAY_TICK);
    return TSK.kind[eid] as number;
  }

  it('дичь в соседней loc1 (близко) → охотник ВЫБИРАЕТ HUNT', () => {
    expect(huntChoiceWithGameAt(1)).toBe(TaskKind.HUNT);
  });

  it('та же дичь в далёкой loc9 (dist 355) → НЕ HUNT: бесконечная погоня невыгодна', () => {
    expect(huntChoiceWithGameAt(9)).not.toBe(TaskKind.HUNT);
  });
});

// ───────────────────────────────────────────────────────────────────────────
// ФЛАГ STRICT_HUNT_PERCEPTION (P-9, D-088): OFF (Б) — глобальный поиск угодий;
// ON (В) — строгий анти-чит (кандидаты из восприятия/памяти).
// ───────────────────────────────────────────────────────────────────────────
describe('Б (флаг OFF по умолчанию): глобальный поиск угодий — охота есть без восприятия/памяти', () => {
  it('дичь в соседней loc — HUNT доступна БЕЗ Perception и БЕЗ памяти (глобальный скан, Б)', () => {
    // При OFF угодья «куда идти» берутся глобально (как было до P-5/Б): охотник знает,
    // где ЕСТЬ живая дичь. Это возвращает охоту на всех seed (гейты Фазы 1/2 зелёные).
    const w = createSimWorld(410 as Seed);
    const eid = placeStalker(w, { loc: 0, hunger: 100, thirst: 5, fatigue: 0, survival: 0.5, food: false });
    placeAnimal(w, 1); // соседняя loc; охотник её НЕ воспринял (evalAt без Perception) и НЕ помнит
    evalAt(w, DAY_TICK);
    expect(STRICT_HUNT_PERCEPTION).toBe(false); // Б: флаг OFF
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT); // глобальный скан нашёл угодье ⇒ HUNT
    expect(TSK.targetLoc[eid]).toBe(1);
  });
});

// ───────────────────────────────────────────────────────────────────────────
// P-5/В (D-088, canary ЗА ФЛАГОМ ON): угодья HUNT — ТОЛЬКО из ЛИЧНОГО ВОСПРИЯТИЯ/
// ПАМЯТИ, НЕ глобальный скан (строгий P-9). Пропускается при OFF (В включит флаг).
// ───────────────────────────────────────────────────────────────────────────
(STRICT_HUNT_PERCEPTION ? describe : describe.skip)('P-5/В (флаг ON): угодья HUNT из восприятия/памяти, НЕ глобальный скан (анти-чит)', () => {
  it('АНТИ-ЧИТ ЛОВИТ СКАН: близкая дичь, которую охотник НЕ воспринял и НЕ помнит → НЕ HUNT', () => {
    // Голодный без еды в loc0; дичь в СОСЕДНЕЙ близкой loc1 (глобальный скан выбрал бы её).
    // БЕЗ Perception (evalAt) и БЕЗ памяти охотник о ней НЕ знает ⇒ HUNT недоступна.
    const w = createSimWorld(400 as Seed);
    const eid = placeStalker(w, { loc: 0, hunger: 100, thirst: 5, fatigue: 0, survival: 0.5, food: false });
    placeAnimal(w, 1); // близко (ребро 0—1), но охотник её не видит и не помнит
    evalAt(w, DAY_TICK);
    // Если бы жил глобальный скан animalLocs — был бы HUNT→loc1. Его нет ⇒ НЕ HUNT.
    expect(TSK.kind[eid]).not.toBe(TaskKind.HUNT);

    // А ВОТ ЕСЛИ он ЛИЧНО помнит это угодье — HUNT доступна (то же состояние мира,
    // разница ТОЛЬКО в личном знании охотника ⇒ угодья идут из памяти, не из скана).
    rememberGrounds(w, eid, 1);
    evalAt(w, DAY_TICK);
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT);
    expect(TSK.targetLoc[eid]).toBe(1);
  });

  it('ВОСПРИЯТИЕ СЕЙЧАС: co-located дичь через Perception → угодье познано → HUNT', () => {
    // Perception наполнит contacts co-located дичью ⇒ TaskSelection ЛИЧНО «увидит» её,
    // запишет угодье и выберет HUNT — БЕЗ предварительного seed памяти (анти-чит-чисто).
    const w = createSimWorld(401 as Seed);
    const eid = placeStalker(w, { loc: 4, hunger: 95, thirst: 5, fatigue: 5, survival: 0.6, food: false });
    placeAnimal(w, 4); // co-located дичь — охотник её РЕАЛЬНО воспринимает
    perceiveEvalAt(w, DAY_TICK);
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT);
    expect(TSK.targetLoc[eid]).toBe(4);
  });

  it('ЭКОНОМИЧЕСКАЯ НЕДООХОТА: пустые выходы к угодью забывают его → охотник переключается', () => {
    // Охотник ПОМНИТ угодье loc4, но дичи там НЕТ (перевыбито). Стоя у пустого угодья,
    // он раз за разом выходит впустую ⇒ вера убывает ⇒ угодье ЗАБЫТО ⇒ HUNT сходит.
    const w = createSimWorld(402 as Seed);
    const eid = placeStalker(w, { loc: 4, hunger: 95, thirst: 5, fatigue: 0, survival: 0.6, food: false });
    rememberGrounds(w, eid, 4); // помнит угодье, но дичи в loc4 нет (никого не селим)
    // Первый выбор: помнит угодье (у себя под ногами) ⇒ HUNT.
    evalAt(w, DAY_TICK);
    expect(TSK.kind[eid]).toBe(TaskKind.HUNT);
    // Повторные пустые выходы стоя у угодья ЭРОДИРУЮТ веру; угодье забывается ⇒ переключение.
    let switched = false;
    for (let t = 0; t < 12 && !switched; t++) {
      evalAt(w, DAY_TICK);
      if ((TSK.kind[eid] as number) !== TaskKind.HUNT) switched = true;
    }
    expect(switched).toBe(true); // не бьётся вечно в пустое угодье — эмерджентная недоохота
  });
});
...
describe('обход полный цикл: ограбление → обход → затухание (MemoryDecay) → возврат к цели', () => {
  it('RobberyMemory метит место охоты; NPC огибает его; по сроку MemoryDecay снимает — NPC возвращается', () => {
    ...
    placeAnimal(w, 2);
    placeAnimal(w, 3);
    rememberGrounds(w, victim, 2, 3); // помнит оба угодья (личный опыт, D-087)
