...
import { setRelation, entitySubject } from './memory';
...
describe('SEARCH: намерение искать артефакт выводится из состояния поля (закон №2)', () => {
  ...
  it('SEARCH сильнее TRADE у спокойного NPC: поле с артефактом И поселение с поводом → SEARCH', () => {
    // W.search(0.7) > W.trade(0.6): при равном гейте safety·needCalm жадность за
    // дорогим хабаром перебивает рутинную торговлю (обоснование веса).
    const w = createSimWorld(507 as Seed);
    placeArtifactField(w, 0);
    placeSettlement(w, 0);
    const eid = placeStalker(w, { loc: 0, home: 0 });
    setInv(w, eid, SURPLUS_INV); // повод торговать тоже есть
    evalAt(w, DAY_TICK);
    expect(TSK.kind[eid]).toBe(TaskKind.SEARCH); // артефакт бьёт торговлю
  });
