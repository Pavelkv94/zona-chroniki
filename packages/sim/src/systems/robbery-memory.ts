 ...
 *
 * ── ИЗОЛЯЦИЯ (голдены Фазы 1 целы) ───────────────────────────────────────────
 * RobberyMemory НЕ регистрируется в registerPhase1Systems и не создаётся worldgen
 * (подключит 2.16). В живом мире стартовая когорта — 'loners' (не хищники, ROB дремлет,
 * D-062), бандитов нет ⇒ `loot/transferred` НЕ эмитится ⇒ система читает 0 таких событий
 * ⇒ 0 записей памяти ⇒ полный no-op. Экспортируется как System из @zona/sim; голдены
 * sim:100days (37a19d72) и пустого мира (481914ae) НЕ сдвигаются.
 */
