...
// ═══════════════════════════════════════════════════════════════════════════
// НАХОДКА — переедание впустую: TaskEffects исполняет EAT ВСЛЕПУЮ; хуже —
// TaskSelection ВЫБИРАЕТ EAT даже у сытого (плоский бонус W.food), и запас
// сгорает по единице/тик без пользы (голод уже 0). Тесты ФИКСИРУЮТ поведение.
// ═══════════════════════════════════════════════════════════════════════════

describe('переедание впустую: еда ТРАТИТСЯ без пользы (характеризующие тесты — см. НАХОДКУ)', () => {
  it('механика: EAT при hunger=0 всё равно списывает единицу; hunger остаётся 0 (чистая ПОТЕРЯ)', () => {
    const w = createSimWorld(110 as Seed);
    const eid = placeActor(w, { loc: 0, hunger: 0, kind: TaskKind.EAT, inventory: [{ item: 'canned', qty: 2 }] });
    execOnce(w);
    // Закон №3 (создание из воздуха) НЕ нарушен — предмет физически ИСЧЕЗ (расход),
    // но конвертации в сытость нет: консерва (nutrition 45) сожжена впустую.
    expect(NEED.hunger[eid]).toBe(0);
    expect(totalItems(inv(w, eid))).toBe(1);
  });

  it('дизайн-НАХОДКА: сытый (hunger=0) в СУХОЙ локации с едой ВЫБИРАЕТ EAT и палит запас КАЖДЫЙ тик', () => {
    // ВНИМАНИЕ: ХАРАКТЕРИЗУЮЩИЙ тест текущего (сомнительного) поведения, НЕ эталон.
    // Причинная цепочка: sEat = W.hunger*hunger + W.food; плоское слагаемое W.food(0.3)
    // делает EAT победителем argmax даже при hunger=0 (бьёт FORAGE 0.11, REST 0.1, SLEEP
    // 0.165 в сухой глуши). Каждый тик → −1 еда, hunger clamp 0 — ЧИСТАЯ потеря запаса.
    // (В ВОДНОМ поселении, напр. дома в Кордоне, DRINK делит ту же базу 0.3 и, т.к. жажда
    //  растёт быстрее голода, ПЕРЕХВАТывает выбор со 2-го тика — там потеря само-ограничена;
    //  но в сухой локации ограничителя нет и сгорает ВЕСЬ стек. См. НАХОДКУ в отчёте.)
    ...
    const burned = foodStart - totalItems(inv(w, eid));
    expect(burned).toBeGreaterThanOrEqual(5); // ~1 консерва/тик СОЖЖЕНА впустую
    expect(NEED.hunger[eid] as number).toBeLessThan(1); // всё это время сталкер СЫТ
    const ate = w.bus.log.some(
      (e) => e.type === 'task/selected'
        && (e.payload as { eid: number }).eid === eid
        && (e.payload as { kind: number }).kind === TaskKind.EAT,
    );
    expect(ate).toBe(true); // причина потери — выбор EAT у сытого
  });
});
...
describe('сценарий выживания: без пополнения запас кончается и сталкер снова голодает', () => {
  it('дома с конечным запасом и без дичи: еда истощается, hunger РЕБАУНДит к критическому, hp падает', () => {
    ...
    // Запас КОНЕЧЕН: 3 консервы исчерпаны за считаные тики (нет охоты 1.10 — не пополнить).
    expect(depletedTick).toBeGreaterThanOrEqual(0);
    expect(depletedTick).toBeLessThan(10);
