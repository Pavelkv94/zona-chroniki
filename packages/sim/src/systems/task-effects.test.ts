 ...
 *    (REST < SLEEP); кламп 0;
 *  - FORAGE: добывает forage_food из обилия локации (леджер item/harvested{forage});
 *    пусто→ноль; истощение→регенерация обилия (возобновляемый ресурс, 5.2);
...
import { Position, Needs, Health, Home, Human, Alive, Task, WorldClock, TaskKind } from '../core/components';
...
import { WEATHER_CODE } from '../core/components';
import { getItem, getLocation } from '../data/index';
...
// Обилие собирательства (5.2) живёт на singleton WorldClock. Минимальные тест-миры
// (createSimWorld без worldgen) его не имеют ⇒ FORAGE-тесты явно ставят носитель.
const CLOCK = WorldClock as unknown as { weather: Uint8Array; weatherSince: Uint32Array };
/** Ставит singleton WorldClock (носитель обилия), возвращает его eid. */
function spawnClock(world: SimWorld): EntityId {
  const eid = spawnEntity(world.ecs);
...
/** Текущее обилие патча `loc` из ресурса на WorldClock (base по умолчанию). */
function abundanceOf(world: SimWorld, clockEid: EntityId, loc: number): number {
  const arr =
    world.resources.get<ReadonlyArray<readonly [number, number]>>('forageAbundance', clockEid) ?? [];
  for (const [l, a] of arr) if (l === loc) return a;
  return getLocation(loc as LocationId).forage;
}
...
describe('FORAGE: добывает forage_food из обилия причинно; голод НЕ гасит напрямую (5.2)', () => {
  it('богатая локация (loc4 forage=0.7): в инвентарь падает floor(YIELD*обилие) forage_food', () => {
    const w = createSimWorld(30 as Seed);
    const clock = spawnClock(w);
    const base = getLocation(4 as LocationId).forage; // 0.7
    const eid = placeActor(w, { loc: 4, hunger: 50, kind: TaskKind.FORAGE });
    execOnce(w);
    const expected = Math.floor(FORAGE_FOOD_YIELD_PER_ABUNDANCE * base);
    expect(expected).toBeGreaterThan(0);
    expect(inv(w, eid)).toEqual([{ item: 'forage_food', qty: expected }]);
    // Голод НЕ тронут FORAGE напрямую — гасится позже через EAT (физичнее, 5.2).
    expect(NEED.hunger[eid]).toBe(50);
    // Патч ИСТОЩЁН пропорц. добыче.
    expect(abundanceOf(w, clock, 4)).toBeCloseTo(base - expected * FORAGE_DEPLETION_PER_FOOD, 6);
  });

  it('причинно из обилия: пусто (Саркофаг loc9 forage=0.05) → НОЛЬ, без леджера', () => {
    const w = createSimWorld(31 as Seed);
    spawnClock(w);
    const eid9 = placeActor(w, { loc: 9, hunger: 50, kind: TaskKind.FORAGE });
    execOnce(w);
    // floor(6*0.05)=0 ⇒ ничего не добыто, инвентарь пуст, НЕТ item/harvested.
    expect(inv(w, eid9)).toEqual([]);
    expect(w.bus.log.filter((e) => e.type === 'item/harvested')).toHaveLength(0);

    // Пропорциональность обилию: богатая loc4 даёт СТРОГО больше бедной loc9(=0).
    const w2 = createSimWorld(31 as Seed);
    spawnClock(w2);
    const eid4 = placeActor(w2, { loc: 4, hunger: 50, kind: TaskKind.FORAGE });
    execOnce(w2);
    expect(totalItems(inv(w2, eid4))).toBeGreaterThan(0);
  });

  it('леджер item/harvested{source:forage} с causedBy = Task.causeEvent', () => {
    const w = createSimWorld(32 as Seed);
    spawnClock(w);
    const eid = placeActor(w, { loc: 2, hunger: 40, kind: TaskKind.FORAGE });
    // Проставим причину задачи (штамп D-030, как TaskSelection): нефиктивный EventId.
    const cause = 4242;
    (Task as unknown as { causeEvent: Uint32Array }).causeEvent[eid] = cause;
    execOnce(w);
    const harv = w.bus.log.filter((e) => e.type === 'item/harvested');
    expect(harv).toHaveLength(1);
    const h = harv[0] as Extract<SimEvent, { type: 'item/harvested' }>;
    expect(h.payload).toMatchObject({ who: eid, item: 'forage_food', source: 'forage' });
    expect(h.payload.qty).toBe(Math.floor(FORAGE_FOOD_YIELD_PER_ABUNDANCE * getLocation(2 as LocationId).forage));
    expect(h.causedBy).toBe(cause);
  });

  it('нет носителя WorldClock (нет среды) ⇒ FORAGE no-op (пустой мир не оживает)', () => {
    const w = createSimWorld(33 as Seed);
    const eid = placeActor(w, { loc: 4, hunger: 50, kind: TaskKind.FORAGE }); // без spawnClock
    execOnce(w);
    expect(inv(w, eid)).toEqual([]);
    expect(NEED.hunger[eid]).toBe(50);
  });
...
// ═══════════════════════════════════════════════════════════════════════════
// FORAGE — ЗАВИСИМОСТЬ ОТ ПОГОДЫ (задача P-5/Б, D-087, закон №2 — из состояния среды)
// ═══════════════════════════════════════════════════════════════════════════

describe('FORAGE: выход зависит от ПОГОДЫ (дождь/туман/гроза < ясно, D-087)', () => {
  /** Выход собирательства в loc4 при коде погоды `weatherCode` (свежий мир). */
  function forageYieldAt(weatherCode: number): number {
    const w = createSimWorld(80 as Seed);
    const clock = spawnClock(w);
    CLOCK.weather[clock] = weatherCode; // погода среды (WorldClock.weather, закон №2)
    const eid = placeActor(w, { loc: 4, hunger: 50, kind: TaskKind.FORAGE });
    execOnce(w);
    return totalItems(inv(w, eid));
  }

  it('дождь/туман/гроза дают СТРОГО меньше собранного, чем ясно (из WorldClock.weather)', () => {
    const clear = forageYieldAt(WEATHER_CODE.clear);
    const rain = forageYieldAt(WEATHER_CODE.rain);
    const fog = forageYieldAt(WEATHER_CODE.fog);
    const storm = forageYieldAt(WEATHER_CODE.storm);
    expect(clear).toBeGreaterThan(0);
    expect(rain).toBeLessThan(clear); // дождь < ясно (задача Б)
    expect(fog).toBeLessThan(clear); // туман < ясно
    expect(storm).toBeLessThan(clear); // гроза < ясно
    expect(storm).toBeLessThanOrEqual(rain); // гроза (0.3) не сильнее дождя (0.5)
  });

  it('выход = floor(YIELD × обилие × погодный_множитель) — точная формула (детерминизм)', () => {
    const base = getLocation(4 as LocationId).forage;
    for (const wt of WEATHER_TYPES) {
      const code = WEATHER_CODE[wt];
      const factor = FORAGE_WEATHER_FACTOR_BY_CODE[code] as number;
      const expected = Math.floor(FORAGE_FOOD_YIELD_PER_ABUNDANCE * base * factor);
      expect(forageYieldAt(code)).toBe(expected);
    }
  });
});

// ═══════════════════════════════════════════════════════════════════════════
// forage_food — ХУДШАЯ еда (задача P-5/Б, D-087): низкокалорийна vs мясо/хлеб/консервы
// ═══════════════════════════════════════════════════════════════════════════

describe('forage_food — низкокалорийная худшая еда (D-087)', () => {
  it('nutrition forage_food снижен и СТРОГО меньше мяса/хлеба/консервов', () => {
    const forage = getItem('forage_food').nutrition ?? 0;
    expect(forage).toBe(10); // снижен (было 15)
    expect(forage).toBeLessThan(getItem('meat').nutrition ?? 0); // мясо (35) сытнее
    expect(forage).toBeLessThan(getItem('bread').nutrition ?? 0); // хлеб (25) сытнее
    expect(forage).toBeLessThan(getItem('canned').nutrition ?? 0); // консервы (45) сытнее
  });

  it('EAT forage_food гасит голод ровно на его (малую) nutrition', () => {
    const w = createSimWorld(81 as Seed);
    const nutrition = getItem('forage_food').nutrition!; // 10
    const eid = placeActor(w, {
      loc: 4,
      hunger: 50,
      kind: TaskKind.EAT,
      inventory: [{ item: 'forage_food', qty: 2 }],
    });
    execOnce(w);
    expect(NEED.hunger[eid]).toBeCloseTo(50 - nutrition, 4);
    expect(inv(w, eid)).toEqual([{ item: 'forage_food', qty: 1 }]); // ушла 1 единица
  });
});

describe('FORAGE: истощение → ноль → регенерация обилия (возобновляемый ресурс, 5.2)', () => {
  it('стоя фуражируя, патч истощается до нуля-выхода за конечное число тиков', () => {
    const w = createSimWorld(34 as Seed);
    const clock = spawnClock(w);
    const eid = placeActor(w, { loc: 4, hunger: 20, kind: TaskKind.FORAGE });
    let harvestTicks = 0;
    for (let i = 0; i < 50; i++) {
      const before = totalItems(inv(w, eid));
      execOnce(w);
      if (totalItems(inv(w, eid)) > before) harvestTicks++;
    }
    // Истощился: выход floor(YIELD*a)=0, дальнейшие тики ничего не дают (пусто→ноль).
    expect(harvestTicks).toBeGreaterThan(0);
    expect(harvestTicks).toBeLessThan(50); // не бесконечная еда из патча
    expect(abundanceOf(w, clock, 4)).toBeLessThan(getLocation(4 as LocationId).forage);
    // Собранное — единственная запись forage_food (слияние, не дубли).
    expect(inv(w, eid)).toEqual([{ item: 'forage_food', qty: totalItems(inv(w, eid)) }]);
  });

  it('регенерация ДЕТЕРМИНИРОВАННА: истощённый патч отрастает к base на FORAGE_REGEN/тик', () => {
    const w = createSimWorld(35 as Seed);
    const clock = spawnClock(w);
    const eid = placeActor(w, { loc: 4, hunger: 20, kind: TaskKind.FORAGE });
    execOnce(w); // один сбор → патч истощён
    const depleted = abundanceOf(w, clock, 4);
    expect(depleted).toBeLessThan(getLocation(4 as LocationId).forage);
    // Уводим фуражира из FORAGE — патч только регенерирует (без истощения).
    (Task as unknown as { kind: Uint8Array }).kind[eid] = TaskKind.REST;
    execOnce(w);
    expect(abundanceOf(w, clock, 4)).toBeCloseTo(depleted + FORAGE_REGEN_PER_TICK, 6);
  });

  it('регенерация до base снимает запись обилия (ресурс возвращается к «полному»)', () => {
    ...
    (Task as unknown as { kind: Uint8Array }).kind[eid] = TaskKind.REST;
    const base = getLocation(4 as LocationId).forage;
    // Прогоняем достаточно тиков, чтобы полностью отрасти (запись должна исчезнуть).
    for (let i = 0; i < 2000; i++) execOnce(w);
    expect(w.resources.has('forageAbundance', clock)).toBe(false);
    expect(abundanceOf(w, clock, 4)).toBe(base);
  });
});
...
describe('кламп и монотонность: восстановление не создаёт нужд из воздуха', () => {
  it('нужда=0 у своего эффекта остаётся 0 (DRINK/REST/SLEEP); ниже 0 не уходит', () => {
  ...
  it('восстановление НИКОГДА не увеличивает нужду и не превышает NEED_MAX (только вниз)', () => {
    ...
    for (const [need, eid] of [
      [NEED.thirst[d], d], [NEED.fatigue[r], r], [NEED.fatigue[s], s],
    ] as const) {
  ...
  it('DRINK — из среды, инвентарь неизменен; FORAGE добавляет forage_food (закон №3)', () => {
    ...
    // FORAGE ДОБАВИЛ forage_food к банке (масса из среды, сорт. по item сохранён).
    const yieldQty = Math.floor(FORAGE_FOOD_YIELD_PER_ABUNDANCE * getLocation(4 as LocationId).forage);
    expect(inv(w, forager)).toEqual([
      { item: 'canned', qty: 2 },
      { item: 'forage_food', qty: yieldQty }, // 'canned' < 'forage_food' — вставка в конец
    ]);
  });
});

// ═══════════════════════════════════════════════════════════════════════════
// СТОЯЩИЙ контракт для SLEEP; FORAGE даёт >0 там, где обилие достаточно
// ═══════════════════════════════════════════════════════════════════════════

describe('стоящий контракт SLEEP; собирательство даёт добычу в обильных угодьях', () => {
  ...
  it('обильное угодье даёт строго >0 forage_food; мёртвая зона (Саркофаг) — ноль', () => {
    // Саркофаг forage=0.05 ⇒ floor(6*0.05)=0 — пусто → ноль (закон №1/№2).
    const w = createSimWorld(151 as Seed);
    spawnClock(w);
    const dead = placeActor(w, { loc: 9, hunger: 50, kind: TaskKind.FORAGE });
    const rich = placeActor(w, { loc: 4, hunger: 50, kind: TaskKind.FORAGE });
    execOnce(w);
    expect(totalItems(inv(w, dead))).toBe(0);
    expect(totalItems(inv(w, rich))).toBeGreaterThan(0);
  });
});
