 ...
 *  - причинность через ШТАМПЫ (D-030, ретрофит 1.8): departed.causedBy =
 *    Task.causeEvent (штамп task/selected); arrived.causedBy = Position.moveCause
 *    (= id departed этого шага). Без скана лога — причины из полей компонентов.
 *
...
import { spawnEntity, addComponent, stampCause } from '../core/ecs';
...
/**
 * Публикует мок-событие `task/selected` (ретрофит 1.8: теперь в union) и ШТАМПУЕТ
 * его id в `Task.causeEvent` носителя — ровно то, что делает TaskSelection при смене
 * задачи (D-030). Movement читает causeEvent из компонента (O(1), без скана лога).
 */
function stampTaskCause(world: SimWorld, eid: EntityId, targetLoc: number): EventId {
  const selId = world.bus.publish({
    type: 'task/selected',
    causedBy: null,
    payload: { eid, kind: TaskKind.FORAGE, targetLoc: targetLoc as LocationId },
  });
  stampCause(Task, 'causeEvent', eid, selId);
  return selId;
}

describe('причинность через штамп: departed.causedBy = Task.causeEvent (D-030)', () => {
  it('штампованный task/selected становится причиной departed (без скана лога)', () => {
    const w = createSimWorld(9 as Seed);
    const eid = placeMover(w, 0, 1);

    // Штампуем Task.causeEvent (как TaskSelection 1.8 при смене задачи).
    const selId = stampTaskCause(w, eid, 1);
    ...
    expect(dep.causedBy).toBe(selId); // из Task.causeEvent, не из лога
  });

  it('Task.causeEvent = 0 (не штамповано) → departed.causedBy = null', () => {
    const w = createSimWorld(9 as Seed);
    const eid = placeMover(w, 0, 1); // placeMover не штампует causeEvent (=0)
    movementScheduler().run(w, 1);
    const dep = moveEvents(w, eid).find((e) => e.type === 'move/departed')!;
...
describe('причинность мультихопа через штампы: task/selected → departed → arrived', () => {
  // 0→5 = [0,1,2,5] (3 хопа). Штамп задачи стабилен на всю ногу (задача не менялась),
  // поэтому КАЖДЫЙ departed цепляется к task/selected, а КАЖДЫЙ arrived — к своему
  // departed (Position.moveCause). Цепочка каждого хопа замкнута без скана лога.
  it('все departed → selId; все arrived → свой departed; цепочка резолвится', () => {
    const w = createSimWorld(11 as Seed);
    const eid = placeMover(w, 0, 5);

    const selId = stampTaskCause(w, eid, 5);
    w.bus.endTick(0 as Tick);
    w.tick = 1 as Tick;
    ...
    // Каждый departed ноги ссылается на один и тот же task/selected (штамп стабилен).
    expect(dep1.causedBy).toBe(selId);
    expect(dep2.causedBy).toBe(selId);
    expect(dep3.causedBy).toBe(selId);
    // Каждое прибытие — к своему departure (Position.moveCause).
    expect(arr1.causedBy).toBe(dep1.id);
    expect(arr2.causedBy).toBe(dep2.id);
    expect(arr3.causedBy).toBe(dep3.id);

    // Полная резолюция хопа от финального arrived до корня: arr3→dep3→selId→null.
    ...
    expect(cursor!.id).toBe(selId); // упирается в task/selected
    expect(cursor!.causedBy).toBeNull(); // корень
    expect(steps).toBe(2); // arr3→dep3→sel
  });
});
