 ...
 * Chronicle экспортируется как System, но в конвейер (registerPhase2Systems) в 3.2 НЕ
 * подключается — вместе с Radio 3.5 / Rumors 3.6 её включат на 3.7 (батч сдвига голденов +
 * запуск fame-петли). На текущем прогоне не гоняется ⇒ голдены Фазы 3 не двигаются
 * (sim:100days fd0bec10, пустой мир 481914ae), EconomyInvariant не затронут (запись массу/деньги
 * не творит; incFame двигает ключ 'fame', дизъюнктный money/inventory, D-067).
 *
 * ```mermaid
 * flowchart TD
