 ...
 * Rumors включена в единый конвейер Фазы 3 (registerPhase3Systems: … Radio → Rumors →
 * Chronicle → Death) вместе с Chronicle/Radio. Голдены ЗАКОННО сдвинулись (молва заполняет лог
 * radio/relayed): sim:100days fd0bec10 → 561cc138, day1 seed42 3c54d141 → f554331d. Пустой мир
 * 481914ae цел (нет слышащих ⇒ no-op). EconomyInvariant не затронут: `radio/relayed` массу/деньги
 * не творит (закон №3), `addMemory` двигает ключ 'memory' — дизъюнктный money/inventory.
 *
 * ── ПЕРФ-КВАДРАТ RUMOR-ПАМЯТИ СНЯТ СТРУКТУРНО (3.8, D-075) ────────────────────
 * D-074 отметил ПЕРФ-ФЛАГ: на ПЛОТНОМ старте (хаб Кордон) ретрансляция КОМБИНАТОРНА, а rumor-
 * память копилась быстрее, чем прунилась MemoryDecay (memMax ~38k записей/NPC к дню 8) ⇒ addMemory
 * пересобирал растущий массив на каждую вставку → квадрат ⇒ sim:100days ~70с → ~650с. Задача 3.8
 * (D-075) вылечила это НЕ логикой Rumors (закон №6 — цикл ниже НЕ тронут), а в САМОМ `addMemory`
 * (memory.ts): он КОНСОЛИДИРУЕТ память по факту (kind, subject, isFirsthand) — слушатель хранит ОДНУ
 * обновляемую запись о факте, а не тысячи копий. Тот же слух ещё раз ОСВЕЖАЕТ salience/tick/причину,
 * не аппендит ⇒ массив 'memory' ограничен числом РАЗЛИЧНЫХ фактов (memMax ~60), квадрат снят, а слух
 * стал и СЕМАНТИЧНЕЕ (NPC помнит «банда у Свалки», а не 10 000 раз одно). Голдены поехали (память
 * иначе сериализуется, event-лог тот же): sim:100days 561cc138 → 0f1ef408, day1 f554331d → 429867e2.
 *
 * ```mermaid
 * flowchart TD
