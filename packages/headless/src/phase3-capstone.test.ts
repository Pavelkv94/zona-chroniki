...
describe('Лог-якорь живого конвейера: число событий закреплено (D-075, ре-пин 5.2/D-085)', () => {
  it('day1 seed42 через полный конвейер Фазы 3 даёт РОВНО 19122 событий (лог бит-в-бит)', () => {
    ...
    // P-5 задача А/D-086 (стоимость погони, равновесие дичи) → 19010→19122 (детерминизм 2×).
    // Дешёвый сторож регрессии лога, комплементарный хэшу 6d4317ab (cli.test).
    expect(world.bus.log.length).toBe(19122);
  });
