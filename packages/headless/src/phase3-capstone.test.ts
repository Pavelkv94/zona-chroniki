...
// ═════════════════════════════════════════════════════════════════════════════
describe('Поведение не изменилось сверх сериализации памяти: лог день-1 не сдвинут (D-075)', () => {
  it('day1 seed42 через полный конвейер Фазы 3 даёт РОВНО 14794 события (лог бит-в-бит)', () => {
    const { world, scheduler } = buildPhase3(42);
    scheduler.run(world, TICKS_PER_DAY);
    expect(world.bus.log.length).toBe(14794);
  });

  it('нарратив уже обилен к концу дня-1 (radio/relayed = 1728 — консолидация лог не проредила)', () => {
    const { world, scheduler } = buildPhase3(42);
    scheduler.run(world, TICKS_PER_DAY);
    const counts: Record<string, number> = {};
    for (const ev of world.bus.log) counts[ev.type] = (counts[ev.type] ?? 0) + 1;
    // Точные числа из D-074/D-075: дедуп памяти НЕ трогает эмиссию событий эфира/молвы/летописи.
    expect(counts['radio/relayed']).toBe(1728);
    expect(counts['radio/message']).toBe(22);
    expect(counts['chronicle/recorded']).toBe(17);
  });
});
