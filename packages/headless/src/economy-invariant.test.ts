...
describe('EconomyInvariant покрывает склады/кассы ПОСЕЛЕНИЙ (закон №3, D-046)', () => {
  ...
  it('baseline t0 ВКЛЮЧАЕТ кассы поселений: Σ money == 79000 (20·2000 + 2·2000 + 15000+20000)', () => {
    // 20 сталкеров × 2000 + 2 торговца × 2000 + казна Кордона 15000 + Ростока 20000.
    // Если бы worldTotals не считал кассы поселений (money на их eid), сумма была бы
    // 44000 — тогда экономический базлайн «недосчитывал» бы физически внесённый капитал.
    const world = createSimWorld(42 as Seed);
    worldgen(world);
    expect(worldTotals(world).money).toBe(79000);
  });

  it('baseline t0 ВКЛЮЧАЕТ склады поселений: bread присутствует, а meat == 0 (старт Ф1 без мяса)', () => {
    const world = createSimWorld(42 as Seed);
    worldgen(world);
    const t = worldTotals(world);
    // bread существует ТОЛЬКО на складах поселений (Кордон 30 + Росток 25 = 55) —
    // его наличие в тоталах доказывает, что склад учтён базлайном (D-046).
    expect(t.items.get('bread' as ItemId)).toBe(55);
    // pm: сталкеры 20 + торговцы 2 + склады 3+5 = 30; ammo_9mm: 320 + 32 + 800 = 1152.
    expect(t.items.get('pm' as ItemId)).toBe(30);
    expect(t.items.get('ammo_9mm' as ItemId)).toBe(1152);
