...
describe('Наблюдатель не участник: экспорт ничего в мире не меняет (D-006/D-080)', () => {
  ...
  it('ГОЛДЕН day1 seed42 = 7cd7db13 НЕ сдвигается массовым экспортом (экспорт вне конвейера)', () => {
    const a = build(42);
    a.run(DAY);
    ...
    expect(hashSnapshot(serialize(a.world)), 'экспорт не осквернил голден-мир').toBe('7cd7db13');
  });
