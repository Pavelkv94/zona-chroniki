...
describe('Наблюдатель не участник: экспорт ничего в мире не меняет (D-006/D-080)', () => {
  ...
  it('ГОЛДЕН day1 seed42 = 1b5beda6 НЕ сдвигается массовым экспортом (экспорт вне конвейера)', () => {
    const a = build(42);
    a.run(DAY);
    ...
    expect(hashSnapshot(serialize(a.world)), 'экспорт не осквернил голден-мир').toBe('1b5beda6');
  });
...
describe('Погибший сталкер становится трупом в глазах наблюдателя (порядок Corpse→Human)', () => {
  it('труп с СОХРАНЁННЫМ именем классифицируется как corpse, не human, и не в счёте людей', () => {
    // К концу дня-1 Зоны кто-то гибнет (бой/голод). Труп сталкера несёт И Human, И Corpse;
    // classifyKind проверяет Corpse ПЕРВЫМ ⇒ 'corpse'. Что он был человеком — доказывает
    // СОХРАНЁННОЕ имя в ResourceStore (у звериного трупа имени нет).
    const a = build(42);
    a.run(DAY);
    const view = exportWorldView(a.world);
    ...
    expect(humanCorpse, 'к концу дня-1 ожидается хотя бы один труп бывшего сталкера').not.toBeNull();
  ...
  it('досье трупа несёт его смерть в recentEvents (участие в собственном entity/died)', () => {
    ...
    a.run(DAY);
    const view = exportWorldView(a.world);
    const deaths = a.world.bus.log.filter((e) => e.type === 'entity/died');
    expect(deaths.length).toBeGreaterThan(0);

    // Труп человека, чья смерть есть в логе — его id смерти обязан всплыть в recentEvents.
    for (const c of view.entities.filter((e) => e.kind === 'corpse')) {
      const detail = exportEntityDetail(a.world, c.eid)!;
      if (detail.name === undefined) continue; // только бывший сталкер (у него имя)
      const death = deaths.find((e) => (e.payload as { eid?: number }).eid === (c.eid as number));
      if (death === undefined) continue;
      expect(
        detail.recentEvents.includes(death.id),
        `смерть ${death.id} сталкера ${c.eid} должна быть в его недавних событиях`,
      ).toBe(true);
      return; // достаточно одного доказанного трупа
    }
    throw new Error('не нашлось трупа сталкера с событием смерти — сценарий не воспроизвёлся');
  });
});
