...
describe('Детерминизм 2× и resume≡continuous С НАРРАТИВОМ (закон №8, D-074)', () => {
  const D3_SEED = 42;
  // 5.2/D-085 (FORAGE→forage_food): день 1 стал мирным (0 смертей ⇒ 0 эфира), молва
  // разгорается со дня 2. Горизонт сдвинут на день, чтобы СПЛИТ пришёлся на НАПОЛНЕННУЮ
  // нарративом середину (день 2 полон эфира/слухов) — resume через живой нарратив нетривиален.
  const TOTAL = 3 * TICKS_PER_DAY; // короткий горизонт (ПЕРФ-ФЛАГ), нарратив обилен ко дню 2-3
  const SPLIT = 2 * TICKS_PER_DAY; // сплит на день 2 (эфир/слухи уже звучат)
