...
describe('exportNames: чистое чтение — голдены целы (D-006/D-080)', () => {
  ...
  it('ГОЛДЕН day1 seed42 = 00dc66c3 не сдвигается вызовом exportNames', () => {
    const a = build(42);
    a.run(DAY);
    expect(hashSnapshot(serialize(a.world))).toBe('00dc66c3');
    exportNames(a.world);
    expect(hashSnapshot(serialize(a.world)), 'exportNames осквернил голден-мир').toBe('00dc66c3');
  });
...
describe('exportNames: корректность (люди/трупы/животные)', () => {
  ...
  it('имя ПОГИБШЕГО сталкера (труп) сохраняется в индексе; животных индекс не несёт', () => {
    const a = build(42);
    // 5.2/D-085: форедж снял голодные смерти дня-1 (день 1 мирный) — первые трупы со дня 3.
    // Горизонт 3 дня гарантирует наличие трупа (интент теста — имя покойника в индексе — цел).
    a.run(3 * DAY);
    const view = exportWorldView(a.world);
    const names = exportNames(a.world);
