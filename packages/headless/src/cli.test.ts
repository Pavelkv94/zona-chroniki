...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → cb104eca (тот же прогон, что npm run smoke)', () => {
    ...
    // cb104eca (мир НЕ изменился, только лог событий длиннее). См. DECISIONS D-042.
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('cb104eca');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → 84359104 (тот же прогон, что npm run sim:100days)',
    () => {
      // Пере-закреплён balance-analyst-сессией: 925aa279 → f4cc990d. Пере-закреплён
      // задачей 2.0 (D-045): леджер-события в логе → f4cc990d → 84359104. Прохождение
      // этого голдена ДОКАЗЫВАЕТ: EconomyInvariant держится весь прогон (runHeadless
      // сверяет массу с леджером раз в игровой день и НЕ бросает) — наследие Фазы 1
      // не творит массу из воздуха (закон №3, D-045).
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('84359104');
    },
    30000,
  );
