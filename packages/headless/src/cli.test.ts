...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 9bc823a7 (тот же прогон, что npm run smoke)', () => {
    ...
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('9bc823a7');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → 0eb70da4 (тот же прогон, что npm run sim:100days)',
    () => {
      ...
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('0eb70da4');
    },
    240000, // 144000 тиков плотного ЖИВОГО конвейера Фазы 2 (2.16b): ~70с изолированно, но под ПАРАЛЛЕЛЬНОЙ нагрузкой всего сьюта до ~130с — берём 2× запас
  );
