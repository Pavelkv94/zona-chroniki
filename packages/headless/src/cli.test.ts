...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 70e9e546 (тот же прогон, что npm run smoke)', () => {
    ...
    // См. DECISIONS D-042/D-045/D-046.
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('70e9e546');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → ee2ef84c (тот же прогон, что npm run sim:100days)',
    () => {
      ...
      // задачей 2.2: поселения+торговцы → 84359104 → ee2ef84c. Прохождение этого голдена
      // ДОКАЗЫВАЕТ: EconomyInvariant держится весь прогон, ВКЛЮЧАЯ склады/кассы поселений
      // в базлайне t0 (runHeadless сверяет массу с леджером раз в игровой день и НЕ
      // бросает) — ни наследие Фазы 1, ни поселения не творят массу из воздуха (№3, D-045).
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('ee2ef84c');
    },
    30000,
  );
