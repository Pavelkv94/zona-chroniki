...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 74211540 (тот же прогон, что npm run smoke)', () => {
    ...
    // ⇒ 1d52f17d → 74211540.
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('74211540');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → b64691c7 (тот же прогон, что npm run sim:100days)',
    () => {
      ...
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('b64691c7');
    },
