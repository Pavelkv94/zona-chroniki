...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 429867e2 (тот же прогон, что npm run smoke)', () => {
    ...
    // со дня 3), животные больше не гибнут от жажды (graze-first + миграция к воде). Лог
    // день-1 14794→19010, хэш 1b5beda6 → 00dc66c3 (детерминизм 2×).
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('00dc66c3');
  });
  ...
  it(
    ...
    () => {
      ...
      // животные не гибнут от жажды, охота калибрована) ⇒ 722409ac → 4b63bfdf (детерминизм 2×;
      ...
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('4b63bfdf');
    },
