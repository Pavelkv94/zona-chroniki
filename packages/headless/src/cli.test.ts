...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 429867e2 (тот же прогон, что npm run smoke)', () => {
    ...
    // хэш 00dc66c3→6d4317ab (детерминизм 2×).
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('6d4317ab');
  });
  ...
  it(
    ...
    () => {
      ...
      // животные не гибнут от жажды, охота калибрована) ⇒ 722409ac → 04d4f7e4 (детерминизм 2×;
      ...
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('04d4f7e4');
    },
