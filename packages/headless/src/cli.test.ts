...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('десять дней (14400 тиков) прогоняются без NaN и дают валидный хэш живого мира', () => {
    ...
    expect(r.events).toBeGreaterThan(0);
  }, 120000); // запас под параллельную нагрузку: живой мир стал плотнее (задача 2.6, TRADE)
  ...
  it(
    ...
    120000, // 144000 тиков плотного мира (2.6 TRADE) под параллельной нагрузкой сьюта — запас от флейков
  );
  ...
  }, 120000); // 6 плотных 10-дневных прогонов под параллельной нагрузкой сьюта — запас от флейков (2.6)
