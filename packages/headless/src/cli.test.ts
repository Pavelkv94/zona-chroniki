...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 429867e2 (тот же прогон, что npm run smoke)', () => {
    ...
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('429867e2');
  });
  ...
  it(
    ...
    () => {
      ...
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('0f1ef408');
    },
    120000, // D-075: 100 дней ~60с после снятия квадрата rumor-памяти; ~2× запас под параллельную нагрузку сьюта. Мир/хэш от таймаута не зависят (D-006).
  );
