...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 1d52f17d (тот же прогон, что npm run smoke)', () => {
    ...
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('1d52f17d');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → 2fa78c11 (тот же прогон, что npm run sim:100days)',
    () => {
      ...
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('2fa78c11');
    },
