...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 429867e2 (тот же прогон, что npm run smoke)', () => {
    ...
    // пере-сериализация со старым набором полей воспроизводит 429867e2 — поведение не тронуто).
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('1b5beda6');
  });
  ...
  it(
    ...
    () => {
      ...
      // ЧИСТО СХЕМНО 0f1ef408 → 722409ac (events 601468 те же, лог байт-в-байт; поведение не тронуто).
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('722409ac');
    },
