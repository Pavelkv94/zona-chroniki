...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('десять дней (14400 тиков) прогоняются без NaN и дают валидный хэш живого мира', () => {
    ...
    expect(r.events).toBeGreaterThan(0);
  }, 15000); // запас под параллельную нагрузку: живой мир стал плотнее (задача 2.6, TRADE)

  // ── ГОЛДЕНЫ живого CLI (закреплены задачей 1.12; core-голден 481914ae не тронут) ─
  it('ГОЛДЕН: day=1 seed=42 → 165688eb (тот же прогон, что npm run smoke)', () => {
    ...
    // Пере-закреплён задачей 2.6: TaskSelection научился выбирать TRADE — NPC со
    // спросом/избытком в спокойную минуту идут к поселению (task/selected kind=TRADE);
    // новые задачи/переходы сдвигают историю → 70e9e546 → 165688eb. Trade-ИСПОЛНЕНИЕ
    // не в конвейере (D-047), сделки не эмитятся — двигает только выбор задач.
    // См. DECISIONS D-042/D-045/D-046/D-056.
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('165688eb');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → 37a19d72 (тот же прогон, что npm run sim:100days)',
    () => {
      ...
      // 2.6: TaskSelection выбирает TRADE (NPC ходят к поселениям торговать) → ee2ef84c
      // → 37a19d72. Прохождение этого голдена ДОКАЗЫВАЕТ: EconomyInvariant держится весь
      // прогон, ВКЛЮЧАЯ склады/кассы поселений в базлайне t0 (runHeadless сверяет массу с
      // леджером раз в игровой день и НЕ бросает) — ни наследие Фазы 1, ни поселения, ни
      // выбор TRADE не творят массу из воздуха (№3, D-045; Trade-исполнения нет в
      // конвейере, D-047 — двигается только распорядок задач).
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('37a19d72');
    },
    60000, // 144000 тиков плотного мира (2.6 TRADE) под параллельной нагрузкой сьюта — запас от флейков
  );
  ...
  }, 60000); // 6 плотных 10-дневных прогонов под параллельной нагрузкой сьюта — запас от флейков (2.6)
  ...
  }, 60000); // 4 полных живых прогона: под нагрузкой полного сьюта дефолтные 5000мс мало (флейки).
});
...
describe('D-006: метрики, замер времени и режим лога не касаются состояния мира', () => {
  ...
  }, 40000); // 5 кейсов × 3 профиля прогонов; запас под параллель + плотнее мир (2.6 TRADE)
