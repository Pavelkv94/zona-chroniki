...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → 675e1485 (тот же прогон, что npm run smoke)', () => {
    ...
    // 675e1485. См. DECISIONS D-042/D-045/D-046/D-056/D-064.
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('675e1485');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → 626a8329 (тот же прогон, что npm run sim:100days)',
    () => {
      ...
      // PopulationInflux оживают → 37a19d72 → 626a8329. Прохождение этого голдена
      // ДОКАЗЫВАЕТ: EconomyInvariant держится весь прогон, ВКЛЮЧАЯ склады/кассы
      // поселений (базлайн t0), реальные сделки Trade (конс. перевод) и приток
      // item/broughtIn (runHeadless сверяет массу с леджером раз в игровой день и НЕ
      // бросает) — ничего не творится из воздуха (№3, D-045).
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('626a8329');
    },
    120000, // 144000 тиков полного конвейера Фазы 2 (17 систем) под параллельной нагрузкой сьюта — запас от флейков
  );
