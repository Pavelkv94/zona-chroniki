...
describe('runHeadless: одна история из одного seed (закон №8)', () => {
  ...
  it('ГОЛДЕН: day=1 seed=42 → e04c0d77 (тот же прогон, что npm run smoke)', () => {
    ...
    expect(runHeadless({ ...base, days: 1, seed: 42 }).snapshotHash).toBe('e04c0d77');
  });

  it(
    'ГОЛДЕН: day=100 seed=42 → 925aa279 (тот же прогон, что npm run sim:100days)',
    () => {
      expect(runHeadless({ ...base, days: 100, seed: 42 }).snapshotHash).toBe('925aa279');
    },
    30000,
  );
