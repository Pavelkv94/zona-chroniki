...
describe('Ожившие петли Фазы 2 работают: поля рождают/собирают, бандиты грабят (D-065)', () => {
  for (const seed of SEEDS) {
    describe(`seed ${seed}`, () => {
      ...
      it('ROB работает: loot/transferred > 0 (бандиты-хищники грабят одиночек)', () => {
        // Фракция bandits (predatory, D-062) выбирает ROB ⇒ реальные грабежи.
        expect(r.counts['loot/transferred'] ?? 0).toBeGreaterThan(0);
      });

      it('Export пока СУХ: 0 item/exported (собранные артефакты не доходят до склада — хвост 2.16c)', () => {
        // Export вывозит артефакты СО СКЛАДА ПОСЕЛЕНИЯ. NPC собирают артефакты, но за
        // 30 дней не доносят/не продают их поселению ⇒ склад пуст на артефакты ⇒ кран
        // сух. Замыкание цепи SEARCH→несёт→продаёт поселению — балансировка 2.16c.
