...
describe('Ожившие петли Фазы 2 работают: поля рождают/собирают, бандиты грабят (D-065)', () => {
  ...
  // ── АГРЕГАТ по seeds: Export — петля РАБОТАЕТ; ROB — балансовый хвост (P-8/D-085) ──
  // Эмерджентные петли (грабёж требует видимого одиночки, экспорт — доноса артефакта до
  // живого поселения) хрупки к балансу на ОТДЕЛЬНОМ seed, поэтому НЕ фиксируем побайтно
  // per-seed; порог ≥2/3 ловит регрессию, устойчив к хрупкости. Export: 2/3.
  ...
  // БАЛАНС-WATCH (наблюдение, НЕ инвариант — как прей-хвост render.test).
  it('ROB эмерджентность — хвост P-8 (D-085): форедж пацифицировал ранний мир, грабежи редки', () => {
    // 5.2/D-085 (FORAGE→forage_food): фуражировка успокоила ранний мир — сталкеры кормятся
    // локально, реже бродят и пересекаются с бандитами ⇒ ЭМЕРДЖЕНТНЫЙ грабёж за 30 дней упал
    // до 0/3 seeds (было ≥2/3 до 5.2). Это БАЛАНСОВЫЙ ХВОСТ (P-8), не дефект МЕХАНИЗМА: сам
    // грабёж (грабитель Task=ROB бьёт слабую цель → loot/transferred, causedBy=resolved)
    // синтетически ПОКРЫТ в systems/encounters-rob.test.ts (D-060/D-049). Пинуем текущую
    // эмерджентность как НАБЛЮДЕНИЕ: возврат грабежей (баланс/логика hunter-targeting,
    // недоохота, оживление бродяжничества) ФЛИПНЕТ этот пин и заставит осознанно пере-осмыслить.
    expect(seedsWithLoot).toBe(0);
  });
