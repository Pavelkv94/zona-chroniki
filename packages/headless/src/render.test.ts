...
describe('БАЛАНС Фаза1: спираль смягчена (остаток выживает), прей-хвост для Фазы 2', () => {
  it('прей-база всё ещё выбивается к 30 дню (seed 42/7/999) — ЛОГИКА-хвост Фазы 2', () => {
    ...
    // 5.2/D-085 (водопой-миграция): thirst больше НЕ компаундит охоту — часть стад
    // переживает до дня 30 неравномерно по seed (42→~12, 7→0, 999→0): вода спасла тех, кто
    // раньше дох от жажды ДО того, как их выбьют. Стада всё равно ТЯЖЕЛО прорежены от старта
    // (~35 особей → ≤ горстки): ЛОГИКА-хвост охоты (min-eid ганг/недоохота) остаётся Фазой 2.
    // Балансовое НАБЛЮДЕНИЕ (не инвариант): фиксируем «стада ДЕЦИМИРОВАНЫ к дню 30» (≤15 из ~35).
    const DECIMATED_MAX = 15; // тяжело прорежены (было ~35); водопой 5.2 поднял верхнюю границу
