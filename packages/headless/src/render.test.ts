...
describe('живой мир за 10 дней: события растут, есть охота/смерти/погода (seed=42)', () => {
  ...
  it('население МЕНЯЕТСЯ: часть сталкеров гибнет (живых людей < 20 стартовых)', () => {
    ...
    expect(humans.size).toBe(20); // worldgen заселил 20 сталкеров
    expect(aliveHumans).toBeLessThan(20); // мир прожил: были потери
  });
...
// ── ДЕГРАДАЦИЯ-WATCH для гейта 1.13 (наблюдение, НЕ инвариант 1.12) ───────────
// Пиннит ТЕКУЩУЮ (несбалансированную) траекторию живого мира как якорь: 1.13
// (balance) обязана её УЛУЧШИТЬ, и тогда этот блок ОСОЗНАННО обновится/удалится.
// Сейчас документирует НАХОДКУ QA: прей-база (животные) вымирает, а популяция
// сталкеров монотонно тает (нет ни размножения людей, ни притока извне).
describe('НАХОДКА 1.13: живой мир скатывается к вымиранию (якорь текущего баланса)', () => {
  it('животные (прей-база) вымирают к 30 дню при seed 42/7/999 (охота ≫ приплод)', () => {
    for (const seed of [42, 7, 999]) {
      ...
      // ЯКОРЬ: сейчас стада вымирают полностью. Если 1.13 это исправит —
      // тест должен покраснеть и быть осознанно ослаблен (это ЖЕЛАЕМАЯ регрессия).
      expect(aliveAnimals, `seed=${seed}: стада вымерли к 30 дню (баланс 1.13)`).toBe(0);
    }
  }, 30000);

  it('популяция сталкеров монотонно тает: к 100 дню seed=42 не остаётся живых', () => {
    const { world, run } = build(42);
    run(100 * TICKS_PER_DAY);
    const humans = new Set(eidsWith(world, 'human'));
    const alive = new Set(eidsWith(world, 'alive'));
    const aliveHumans = [...alive].filter((e) => humans.has(e)).length;
    // ЯКОРЬ находки: мир к 100 дню — «кладбище» (0 живых людей, 0 животных).
    expect(aliveHumans, 'seed=42: сталкеры вымерли к 100 дню — цель 1.13 переломить').toBe(0);
  }, 30000);
});
