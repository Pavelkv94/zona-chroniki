...
describe('слежение камеры — презентация вида (задача 4.7, закон №8: воркеру не шлём)', () => {
  it('setFollowing(true) при ВЫБРАННОЙ сущности → следим; воркеру ничего не уходит', () => {
    useUiStore.getState().init(42 as Seed);
    useUiStore.getState().inspect(9 as EntityId); // наблюдатель выбрал сущность
    bridge.posted.length = 0;
    useUiStore.getState().setFollowing(true);
    expect(useUiStore.getState().following).toBe(true);
    expect(bridge.posted).toEqual([]); // слежение — чистый ВИД, тик мира не трогаем
  });

  it('setFollowing(true) БЕЗ выбора → остаётся false (вести некого)', () => {
    useUiStore.setState({ ...FRESH, selectedEid: null, following: false });
    useUiStore.getState().setFollowing(true);
    expect(useUiStore.getState().following).toBe(false);
  });

  it('setFollowing(false) выключает слежение даже при активном выборе', () => {
    useUiStore.setState({ ...FRESH, selectedEid: 5 as EntityId, following: true });
    useUiStore.getState().setFollowing(false);
    expect(useUiStore.getState().following).toBe(false);
  });

  it('clearSelection снимает слежение (цель потеряна — вести нечего)', () => {
    useUiStore.setState({ ...FRESH, selectedEid: 7 as EntityId, following: true });
    useUiStore.getState().clearSelection();
    expect(useUiStore.getState().selectedEid).toBeNull();
    expect(useUiStore.getState().following).toBe(false);
  });

  it('init нового мира сбрасывает слежение (прошлая цель не актуальна)', () => {
    useUiStore.setState({ ...FRESH, selectedEid: 3 as EntityId, following: true });
    useUiStore.getState().init(42 as Seed);
    expect(useUiStore.getState().following).toBe(false);
    expect(useUiStore.getState().selectedEid).toBeNull();
  });
