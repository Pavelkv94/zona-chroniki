...
export interface FactionData {
  ...
  /**
   * СТРАТЕГИЧЕСКАЯ диспозиция фракции (Фаза 5, задача 5.1, §4.2): как фракция ведёт
   * себя на карте угроз (стратегический AI 5.9 будет читать; в 5.1 — только КОНТЕНТ):
   *  - `'defensive'` — обороняется, держит территорию, не ищет конфликта;
   *  - `'aggressive'` — расширяется/грабит, инициирует налёты (RAID);
   *  - `'crusader'` — «крестоносцы» (Долг): приоритет ЗАЧИСТКЕ мутантов (SWEEP), а
   *    не людей (§4.2). Поле ДАННЫХ (закон №10): код стратегии читает диспозицию из
   *    контента, а НЕ хардкодит id 'duty'. Опущено ⇒ фракция без выраженной стратегии
   *    (нейтральная). Никто НЕ читает поле до 5.9 — чистая контент-запись.
   */
  readonly stance?: 'defensive' | 'aggressive' | 'crusader';
}
...
/**
 * Запись БОЛЕЗНИ (`diseases.json`, Фаза 5, задача 5.1, закон №10 — болезни это
 * КОНТЕНТ; код оперирует абстрактными id, а не хардкодит «простуду»). `id` —
 * абстрактная ссылка (компонент `Sickness.disease` хранит СЫРОЙ ui8-код в снапшоте;
 * маппинг кода↔id вводит система болезней 5.8), `key` — стабильная строка контента.
 *
 * Все параметры — ПРИЧИННЫЕ (закон №2): болезнь возникает/течёт из СОСТОЯНИЯ мира
 * (контакт с больным, холод, накопленная экспозиция), а НЕ из «X% шанс». Ставки/
 * пороги МЕХАНИКИ болезни (сколько экспозиции даёт контакт, порог заражения) живут
 * в /sim/balance/disease (закон №7); здесь — только СВОЙСТВА самой болезни-контента.
 */
export interface DiseaseData {
  readonly id: string;
  /** Стабильная строка-ключ болезни (ссылка на контент, закон №10). */
  readonly key: string;
  /**
   * Заразность 0..1 — доля экспозиции, передаваемой при контакте с больным носителем
   * (масштабирует EXPOSURE_PER_SICK_CONTACT в 5.8). 0 = не передаётся между людьми
   * (только от среды, напр. холод), 1 = максимально контагиозна.
   */
  readonly transmissibility: number;
  /**
   * Переносится ХОЛОДОМ (`true` ⇒ холодная погода копит экспозицию к этой болезни —
   * простуда). Причинный источник из СРЕДЫ (закон №2), а не заражение от человека.
   */
  readonly coldBorne: boolean;
  /**
   * Скорость роста тяжести (`Sickness.severity`) за тик без лечения (>0). Болезнь
   * без TREAT прогрессирует детерминированно (закон №2), домножается на длительность
   * шага системы болезней (как ставки нужд «за тик»).
   */
  readonly severityRate: number;
  /**
   * Летальность 0..1 — множитель урона здоровью от тяжести болезни (тяжёлая инфекция
   * убивает быстрее лёгкой простуды). Реальная смертность (закон №1), а не «X% смерть».
   */
  readonly lethality: number;
  /**
   * Тиков до выздоровления (целое >0): при экспозиции ниже порога заражения тяжесть
   * спадает и через `recoveryTicks` носитель здоров (HEALTHY). Инкубация/течение —
   * детерминированный срок, а не бросок кости (закон №2/№8).
   */
  readonly recoveryTicks: number;
}

/** Корневая структура `diseases.json` (закон №10). */
export interface DiseasesData {
  readonly diseases: readonly DiseaseData[];
}
